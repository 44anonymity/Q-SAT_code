...
from modules.mixers.dmaq_qatten_atten import DMAQ_QattenMixer_Atten
...
class QPLEX_attenLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.mixer = DMAQ_QattenMixer_Atten(args)
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        if mixer is not None:
            ans_chosen, q_attend_regs, head_entropies, l1_loss = \
                mixer(chosen_action_qvals, hidden_state[:, :-1], batch["state"][:, :-1], is_v=True)
            ans_adv, _, _, _ = mixer(chosen_action_qvals, hidden_state[:, :-1], batch["state"][:, :-1], actions=actions_onehot,
                                    max_q_i=max_action_qvals, is_v=False)
            chosen_action_qvals = ans_chosen + ans_adv
          

            if self.args.double_q:
                target_chosen, _, _ , _ = self.target_mixer(target_chosen_qvals, hidden_state[:, :-1], batch["state"][:, 1:], is_v=True)
                target_adv, _, _, _ = self.target_mixer(target_chosen_qvals, hidden_state[:, :-1], batch["state"][:, 1:],
        ...
        loss = (masked_td_error ** 2).sum() / mask.sum() + q_attend_regs + self.args.regularization * l1_loss
